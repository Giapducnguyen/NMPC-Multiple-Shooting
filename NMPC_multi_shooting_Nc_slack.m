...
% Weights
Q = diag([1, 5, 0.1]);  % on reference tracking
R = diag([0.5, 0.1]);   % on control increment
...
nlp_prob = struct('f', obj, 'x', OPT_variables,...
                    'g', [g_continuity; g_control; g_du], 'p', P);
...
args.lbg(n_state*(Np+1)+n_control*Nc+1 : n_state*(Np+1)+n_control*Nc+2*n_control*Nc) ...
    = kron(ones(Nc,1), [dv_min; domega_min; -Inf; -Inf]);
...
Draw_MPC_point_stabilization_v1 (t,xx,xx1,u_cl,xs,Np,rob_diam)
